...
func TestAddWordToSet(t *testing.T) {
	...
	setsbGroup.POST("/:setId/words", setsController.AddWord)
	...
	t.Run("Set Not Found", func(t *testing.T) {
		payload := request.AddWordRequest{
			Words: []int{1, 2},
		}
	...
	t.Run("No Words To Add", func(t *testing.T) {
		payload := request.AddWordRequest{
			Words: []int{},
		}
		jsonData, _ := json.Marshal(payload)
	...
	t.Run("Success Add Words", func(t *testing.T) {
		payload := request.AddWordRequest{
			Words: []int{1, 7},
		}
		jsonData, _ := json.Marshal(payload)
